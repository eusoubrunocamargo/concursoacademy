...
export default function SlotContainer({ children, days, setOpenDiagnosis }) {

    const { fetchQuestions } = useDiagnosis();
    const daysUntilReview = calcDaysUntilReview(children.created_at, days);
    ...
    const handleOpenNewDiagnosis = async () => {
        ...
        const success = await fetchQuestions(children.subtopic_id);
        if (!success) return;
    ...
    return (
        <div className={styles.slotContainer}>
            ...
            <div className={styles.slotContainer__title}>
                ...
                <span className={styles.slotContainer__title__text}>{children.subtopics.name}</span>}
            </div>
